...
export default function EpisodesPage() {
  const { episodes: eps, loading, error } = useEpisodes();
  const [activeEp, setActiveEp] = useState(null);
  const [search,   setSearch]   = useState("");
  const [visible,  setVisible]  = useState(9); // nb d'épisodes affichés

  useEffect(() => {
    if (eps.length > 0 && !activeEp) setActiveEp(eps[0]);
  }, [eps]);

  // Reset visible quand la recherche change
...
return (
  <div style={{ background: COLORS.dark, height: "100vh", display: "flex", flexDirection: "column" }}>

    {/* ── Header ─────────────────────────────────────────────────── */}
    <div style={{ background: "linear-gradient(180deg, #0d1f3c 0%, #0A1628 100%)", padding: "24px 32px", borderBottom: "1px solid rgba(255,255,255,.1)", flexShrink: 0 }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ color: COLORS.pink, fontSize: 11, fontWeight: 800, letterSpacing: 3, marginBottom: 4 }}>🎬 TOUS LES ÉPISODES</div>
          <h1 style={{ color: "#fff", fontSize: 28, fontWeight: 900, margin: 0, fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ position: "relative" }}>
          <input value={search} onChange={e => { setSearch(e.target.value); setVisible(9); }}
            placeholder="Rechercher un épisode..."
            style={{ background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.12)", color: "#fff", padding: "10px 16px 10px 36px", borderRadius: 8, fontSize: 13, width: 220, outline: "none" }} />
          <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,.3)" }}>🔍</span>
        </div>
      </div>
    </div>

    {/* ── Player + description — fixe, ne scrolle pas ────────────── */}
    <div style={{ flexShrink: 0, padding: "24px 32px", maxWidth: 1200, margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 28 }}>
      {/* Lecteur vidéo */}
    ...
    {/* ── Grille épisodes — scrollable, prend l'espace restant ────── */}
    <div style={{ flex: 1, overflow: "hidden", padding: "0 32px 24px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
      ...
      {/* Container scrollable — scrollbar cachée */}
      <div style={{ height: "100%", overflowY: "scroll", scrollbarWidth: "none", msOverflowStyle: "none" }}>
        <style>{`.episodes-scroll::-webkit-scrollbar { display: none; }`}</style>
        <div className="episodes-scroll" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {displayed.map(ep => (
            <div key={ep.videoId} onClick={() => setActiveEp(ep)}
              style={{ background: activeEp?.videoId === ep.videoId ? "rgba(255,255,255,.07)" : "#111827", borderRadius: 10, overflow: "hidden", cursor: "pointer", border: activeEp?.videoId === ep.videoId ? `1px solid ${COLORS.pink}` : "1px solid rgba(255,255,255,.06)", transition: "all .2s" }}>
